# Morph decision rules (Agent)

Author: xiesq, 2026-07-15

**How to use.** Read this file during main SKILL.md §Morph Pair Planning, **before** writing any Tool JSON `add` / `set`. Infer audience, purpose, and narrative from the user's topic; emit a single `brief.md` that the main recipes will consume. A morph arc without a narrative spine collapses into "slide with motion" instead of "story with motion" — the planning below prevents that.
...
2. **Vary by project** → Browse style-catalog families, avoid repeating recent styles
