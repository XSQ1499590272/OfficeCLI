...
2. **Respect user-specified hex** → if the brief names a brand color, scan `references/style-catalog.md` Quick Lookup for the nearest hex trio; do not force-fit the mood label.
...
Use the style catalog for inspiration (palette + signature gesture), **not** for coordinates — demo coordinates are hand-tuned for demo content.
...
**Shape types** via props `preset=` (confirm with `{{OFFICE_HELP_TOOL}}`): `ellipse | rect | roundRect | triangle | diamond | star5 | hexagon`. Full list from Help pptx shape.

### Naming (SKILL.md is authoritative)

Three-prefix system — `!!scene-*` / `!!actor-*` / `#sN-*`. Source of truth: main SKILL.md §What is Morph?. In Tool JSON, write `name` values literally — no shell history expansion on `!!`, no comment-leading issues on `#`.
...
1. **Establish previous-slide actors** on the new slide (same names; geometry may evolve).
...
Shapes are numbered sequentially on each slide: `shape[1]`, `shape[2]`, `shape[3]`... When `transition=morph` is applied, the host may auto-prefix `!!` to names — **prefer `@name=` selectors** (see main SKILL.md §Known Issues M-1). Index paths remain useful for debugging after clone-equivalent rebuilds.

### Index behavior

- **On creation:** Shapes added in order get increasing indices.
- **After cloning / replaying actors:** New slide inherits shapes with identical indices when order matches.
- **After adding to a cloned slide:** New shapes get the next available index.
- **After modifying:** Index stays the same.

### Pattern for deterministic replay

```
Slide 1: 6 actors + 2 content = 8 shapes total
Slide 2: Replay actors (8) → Ghost content (shape[7-8]) → Add new (shape[9+])
Slide 3: Replay (10) → Ghost content (shape[9-10]) → Add new (shape[11+])
```

**Formula:** Next slide's first new shape index = Previous slide's total shape count + 1.

**Debugging:** Run `get` on `/slide[N]` with `depth 1` to inspect actual indices.
...
1. Slides 2+ must have `transition=morph` (set on create or via `set` on `/slide[N]`).
...
Format: `EFFECT[-DIRECTION][-DURATION][-TRIGGER]`. Confirm presets with `{{OFFICE_HELP_TOOL}}` for pptx animation.

---

## 7) Style References

52 visual style families — see `references/style-catalog.md`. Lookup workflow is in main SKILL.md §Style library lookup. Key rule: **learn the approach, do not copy coordinates** (demo builds have known typesetting bugs).
