...
static partial class CommandBuilder
{
    private static Command BuildViewCommand(Option<bool> jsonOption)
    {
        ...
        var clipOpt = new Option<string?>("--range") { Description = "将输出限制到指定区域。Screenshot：xlsx 单元格范围（'Sheet1!A1:C3' 或 '/Sheet1/A1:C3'）或元素 data-path（如 '/slide[1]/shape[@id=N]'、'/body/table[1]'），PNG 会裁剪至目标边界；text（仅 xlsx）：单元格范围或单个单元格，只输出对应的行/cell，适合大型 sheet。它不是 set 的字符偏移 `range=` 参数，后者用于格式化文本 span（如 3:7）。" };
