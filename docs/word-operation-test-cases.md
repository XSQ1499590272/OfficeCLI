...
本轮整理后的规模统计：操作/场景条目共 364 条，其中已有单元覆盖 102 条、已有自动覆盖 95 条、部分自动覆盖 81 条、示例待改造 14 条、建议人工核验 2 条、CI smoke 1 条，新增待补 69 条；另有 154 条 `WORD-FREEZE-*` 冻结基线映射。这里的条目数不是 xUnit 方法数，也不是展开后的 test case 数，后续实现时一个条目可以对应一个或多个测试方法/数据行。
...
| WORD-UNIT-097 | unit | P1 | `src/officecli/Handlers/Word/WordHandler.Add.Text.cs` | 文本输入特殊处理 | `text` 中真实换行和 tab 生成 `<w:br>/<w:tab>`；`{page}/{pages}` 生成 PAGE/NUMPAGES field；非法 XML 控制字符被拒绝且不污染文档；见 `tests/OfficeCli.Tests/Unit/WordTextInputBoundaryTests.cs` | 已有单元覆盖 |
...
| WORD-UNIT-100 | unit | P0 | `TypedAttributeFallback`, `ParsePropsArray`, `PathAliases` | typed property parser matrix | dotted fallback 已固定元素别名、颜色 `#` 归一化、SDK 已知/未知属性和 nested existing-only 行为；bool/int/length/color/font-size/enum 的完整大小写、空白、非法值和 `key=value`/JSON object 等价矩阵仍待补；见 `tests/OfficeCli.Tests/Unit/WordTypedAttributeFallbackTests.cs` | 部分自动覆盖 |
| WORD-UNIT-101 | unit | P0 | `WordHandler.Navigation`, `WordHandler.Selector` | path/selector grammar boundary | 正常 singleton、1-based index、stable id、`:contains`、`:empty`、`>`、`or` 已有覆盖；未闭合 selector/path 当前抛出可解释 `ArgumentException`，不静默返回空结果已固定；带引号属性值、空 selector 和更多错误码矩阵仍待补；见 `tests/OfficeCli.Tests/Unit/WordSelectorTests.cs`、`tests/OfficeCli.Tests/Unit/WordNavigationTests.cs` | 部分自动覆盖 |
| WORD-UNIT-102 | unit | P0 | `WordHandler.Add`, `InsertPosition` | insert position boundary | append、index=0、尾部 index、before/after stable path、负 index、越界 index、anchor 指向不同 parent 的行为固定；失败后 parent 子节点和 ID 不变 | 待补 |
...
| WORD-UNIT-105 | unit | P1 | `WordHandler.Helpers.Field`, `WordHandler.SeqEval` | field instruction and cached-result parser | PAGE/NUMPAGES/SEQ/REF/PAGEREF/TOC 指令的大小写、开关、空 result、缓存 result、未知 switch、重复 identifier 和 unsupported switch 的当前分支固定 | 待补 |
| WORD-UNIT-106 | unit | P1 | `WordHandler.Helpers.HeaderFooter`, `WordHandler.Set.SectionLayout` | header/footer linkage | default/first/even 的 reference、`titlePg`、`evenAndOddHeaders`、link-to-previous 的继承/解除继承、重复创建和删除最后一个 part 的行为固定 | 待补 |
| WORD-UNIT-107 | unit | P1 | `WordHandler.Add.Structure`, `WordHandler.Set.SectionLayout` | section mutation matrix | 首 section、尾 section、多 section、section 增删后的 body `sectPr` 归属、page size/margin/orientation/columns/direction readback 和 ID/path 稳定 | 待补 |
| WORD-UNIT-108 | unit | P1 | `WordHandler.Set.Element`, table schemas | table row/cell property matrix | row height/header/cantSplit、cell width/fitText/valign/shading/padding、table layout/border/spacing/indent 的 add/set/remove/默认值和非法组合固定 | 待补 |
| WORD-UNIT-109 | unit | P1 | `WordHandler.Add.Media`, `WordHandler.ImageHelpers`, `OleHelper` | media optional property matrix | picture crop/rotation/flip/wrap/position/decorative/hyperlink、OLE display/name/size/content type 的组合 readback、清空和非法范围固定 | 待补 |
...
| WORD-UNIT-112 | unit | P1 | `WordBatchEmitter`, `BatchExecutor` | dump determinism and warning ordering | 同一文档连续 dump 的 item 顺序、资源先后、stable ID、warning 去重和 JSON 序列化稳定；不支持元素明确 warning 而非静默丢失 | 待补 |
| WORD-UNIT-113 | unit | P2 | `WordHandler.HtmlPreview.*` | HTML escaping and data-path safety | CJK/RTL、`&<>"'`、换行/tab、属性值和 path 中 bracket/quote 在 HTML 文本、attribute、data URI 中正确转义；不生成可执行脚本片段 | 待补 |
...
| WORD-CMD-027 | e2e | P1 | `src/officecli/CommandBuilder.Add.cs`, `src/officecli/Handlers/Word/WordHandler.Mutations.cs` | `remove/move/copy` CLI 面 | path remove 后 query 为空、`remove /body/p` 当前删除首个匹配段落、`move --before/--after/--index` 输出和顺序、`add --from --index` copy target parent 输出和 readback、`/body/ole[1]` indexed shorthand 删除与 validate、resident remove 转发及 close 后落盘已固定；resident move/copy 已固定“内存转发成功但当前 `save` 返回 `io_error`、close 后外部顺序不变”的边界，真正持久化仍待源代码修复；见 `tests/OfficeCli.Tests/E2E/WordViewSmokeTests.cs` | 部分自动覆盖 |
...
| ID | 层级 | 优先级 | 依据 | 操作 / 场景 | 建议断言 | 状态 |
|---|---|---:|---|---|---|---|
| WORD-UNIT-117 | unit | P0 | `src/officecli/Core/Watch/WatchNotifier.cs` | watch path/selector 转换 | paragraph、table、row、cell 的 canonical path、alias path、非法 path 分别映射到稳定 selector；不支持类型返回 null，不生成错误 selector | 待补 |
| WORD-UNIT-118 | unit | P0 | `src/officecli/Core/Watch/WatchMark.cs`, `WatchNotifier.cs` | mark wire model 与版本 | mark/unmark/get-marks 的 JSON 字段、正则标记、颜色/范围、重复 mark、removed count、monotonic version 和空列表语义固定；无 watch 与 server reject 必须可区分 | 待补 |
| WORD-UNIT-119 | unit | P1 | `src/officecli/Core/ResidentFlushPolicy.cs` | flush policy parser 与 adaptive EMA | `off`、`each`、`auto`、固定秒数、大小写/空白、越界和非法值的解析结果固定；EMA 和 interval 始终落在最小/最大窗口内 | 待补 |
| WORD-UNIT-120 | unit | P0 | `src/officecli/Core/DocumentLimits.cs`, `DocumentHandlerFactory.cs` | OOXML zip 资源保护 | entry 数量、单 entry 大小、总展开大小、压缩比和 0-byte 文件的边界固定；命中限制返回 `corrupt_file`，不产生半写文件或残留临时资源 | 待补 |
| WORD-UNIT-121 | unit | P1 | `src/officecli/Core/WordNumFmtRenderer.cs` | Word 编号格式渲染矩阵 | decimal/roman/letter/ordinal/cardinal/CJK/Korean/Japanese/Thai/Hebrew/Arabic/hex/enclosed/bullet/none 的边界、0/负数、超大值、未知格式 fallback 固定 | 待补 |
| WORD-UNIT-122 | unit | P1 | `src/officecli/Core/WordPageDefaults.cs`, `LocaleFontRegistry.cs` | 页面默认值与 locale 默认字体 | 默认 page size/margin/方向、twips 合法范围、`zh-CN`/`ja-JP`/`ar-SA`/`en-US` 的字体和 RTL 默认值固定；非法尺寸不溢出 | 待补 |
| WORD-UNIT-123 | unit | P1 | `src/officecli/Core/WordTocBuilder.cs` | TOC 解析与重建 | heading style/outline level、TOC switch（min/max level、hyperlink、no page number）、bookmark 生成、旧 entry 替换和无 TOC 文档 no-op 固定 | 待补 |
...
| WORD-UNIT-126 | unit | P1 | `src/officecli/Core/SchemaKeyNormalizer.cs`, `SchemaOrder.cs` | schema key alias 与 XML 顺序 | canonical key、大小写、legacy alias、重复 key 的优先级固定；同一属性组合输出稳定 schema order，不因 Dictionary 顺序漂移 | 待补 |
| WORD-UNIT-127 | unit | P1 | `src/officecli/Handlers/Word/WordBatchEmitter.Filters.cs` | dump filter 与 synthetic property 过滤 | internal marker、relId、effective、inherited、wrapper 和 field carrier 属性不被错误 replay；支持的 border/shading/padding/theme 子属性按规则 fold，warning 顺序稳定 | 部分自动覆盖 |
...
| WORD-UNIT-129 | unit | P2 | `src/officecli/Handlers/Rendering/BasicRenderers.cs`, `RendererRegistry` | 内置 Word renderer capability | Word renderer 的 format/output/watch capability、错误 input 类型、重复注册幂等、外部 renderer 优先级和 HTML parity 固定 | 待补 |

### 命令入口与跨进程补充

| ID | 层级 | 优先级 | 依据 | 操作 / 场景 | 建议断言 | 状态 |
|---|---|---:|---|---|---|---|
| WORD-CMD-066 | e2e | P1 | `CommandBuilder.Mark.cs`, `WatchNotifier.cs` | mark/list/get-marks/unmark CLI 协议 | explicit path、selector/find、regex、color、`--all`、重复 mark、无 watch、server reject 的 exit/code/stdout/stderr/JSON envelope 和 version 固定 | 部分自动覆盖 |
...
| WORD-FREEZE-118 | WORD-UNIT-099/100/101 | unit | `tests/OfficeCli.Tests/Unit/WordSchemaParityTests.cs`, `tests/OfficeCli.Tests/Unit/WordTypedAttributeFallbackTests.cs`, `tests/OfficeCli.Tests/Unit/WordNavigationTests.cs` | schema operation/property、typed prop parser、path/selector grammar 的当前边界 |
...
| WORD-FREEZE-120 | WORD-UNIT-105/106/107 | unit/functional | `tests/OfficeCli.Tests/Unit/WordFieldParserTests.cs`, `tests/OfficeCli.Tests/Functional/WordSectionRelationshipTests.cs` | field instruction/cache、header/footer linkage、section 增删与 body sectPr 归属 |
| WORD-FREEZE-121 | WORD-UNIT-112/113/114 | unit/functional | `tests/OfficeCli.Tests/Unit/WordDumpDeterminismTests.cs`, `tests/OfficeCli.Tests/Functional/WordHtmlPreviewContractTests.cs` | dump 顺序/warning、HTML 转义/path、validator 诊断结构 |
...
| WORD-FREEZE-137 | WORD-CMD-027/WORD-NEG-014 | e2e | `tests/OfficeCli.Tests/E2E/WordViewSmokeTests.cs` | resident move/copy 的内存顺序、当前 `save` `io_error` 和 close 后外部原顺序 |
...
| WORD-FREEZE-143 | WORD-UNIT-123/WORD-UNIT-124/WORD-UNIT-125 | unit | `tests/OfficeCli.Tests/Unit/WordRefreshAndRenderBackendTests.cs` | TOC rebuild、refresh backend 生命周期、PDF page filter/grid/资源释放 |
| WORD-FREEZE-144 | WORD-UNIT-126/WORD-UNIT-127 | unit | `tests/OfficeCli.Tests/Unit/WordSchemaOrderAndDumpFilterTests.cs` | schema alias/order、synthetic property 过滤、dump fold/warning 顺序 |
| WORD-FREEZE-145 | WORD-UNIT-128/WORD-UNIT-129 | unit | `tests/OfficeCli.Tests/Unit/WordViewSemanticsAndRendererTests.cs` | view text 语义提取、内置 renderer capability、注册幂等与错误 input |
| WORD-FREEZE-146 | WORD-CMD-066/WORD-CMD-067 | e2e | `tests/OfficeCli.Tests/E2E/WordWatchMarkProtocolTests.cs` | mark/list/get-marks/unmark、selection、SSE version/update、断开重连和 pipe 清理 |
