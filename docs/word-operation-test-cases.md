...
| get/view 选项矩阵 | WORD-CMD-044/045/046/047/057 | get depth、view 截断/过滤、HTML page/out、screenshot grid/native 已各有 smoke；更多非法参数、page-count 和 resident lock 仍待补 | 继续补每个高成本分支的非法输入、resident lock 和 JSON/裸输出边界 |
...
| WORD-UNIT-103 | unit | P0 | `WordHandler.Mutations`, `WordHandler.ImageHelpers` | relationship host and relId collision | 已固定 header/footer host 中 picture relationship 不串到 document part、同一 host 的多个 image relId 唯一、主文档不误挂载 host image，且 validate 通过；OLE/hyperlink/chart 的跨 host、共享 part 删除和 copy/remove 冲突矩阵仍待补；见 `tests/OfficeCli.Tests/Unit/WordRelationshipHostTests.cs`、`WordRawContractTests.cs`、`WordComplexRelationshipCleanupTests.cs` | 部分自动覆盖 |
| WORD-UNIT-104 | unit | P1 | `WordHandler`, `WordBatchEmitter.Resources` | package content-type and auxiliary-part lifecycle | 已由样式/编号/设置测试和 fidelity integration 固定 styles、numbering、settings，以及 customXml/customXmlProperties/fontTable/webSettings 的保留与回放；theme/core/app、part content type 全量矩阵、删除/复用和空 part 生命周期仍待补；见 `tests/OfficeCli.Tests/Unit/WordStyleAndNumberingTests.cs`、`WordDocumentSettingsTests.cs`、`tests/OfficeCli.Tests/Integration/WordDocumentFidelityTests.cs` | 部分自动覆盖 |
...
| WORD-UNIT-110 | unit | P1 | `WordHandler.Add.Diagram`, `WordHandler.Add.Media`, complex builders | complex object validation boundary | 已固定 chart 缺 data、diagram 缺 source、watermark 坏 rotation、picture/OLE/shape/textbox/equation 的最小输入与失败后不留可查询节点；坏 chart data、未知 property、复杂 child 部分失败和 dangling relationship 仍待补；见 `tests/OfficeCli.Tests/Unit/WordChartDiagramPreviewTests.cs`、`WordComplexObjectTests.cs`、`WordErrorBoundaryTests.cs`、相关 functional contract | 部分自动覆盖 |
| WORD-UNIT-111 | unit | P1 | `WordHandler.Helpers.FindReplace`, `WordHandler.FormFields` | token and placeholder boundary | 已固定普通文本、跨 run、跨 hyperlink、regex backreference、重复 token、空 replacement 和未替换边界；跨 field/note part、formfield placeholder、转义花括号和重复 key 的完整矩阵仍待补；见 `tests/OfficeCli.Tests/Unit/WordRawAndFindReplaceTests.cs`、`WordFieldParserTests.cs`、相关 form/field contract | 部分自动覆盖 |
...
| WORD-UNIT-114 | unit | P1 | `WordHandler.Validate`, OpenXML validator adapter | validation diagnostic normalization | 已固定健康文档空结果、schema 错误的 `type/description/part/path` 映射、dangling header reference 的 actionable 诊断，以及重复调用的顺序/字段稳定性；多个错误排序、warning 与 error 分离、重复诊断去重仍待补；见 `tests/OfficeCli.Tests/Unit/WordValidationAndRawXmlTests.cs`、`WordChartDiagramPreviewTests.cs` | 部分自动覆盖 |
| WORD-UNIT-115 | unit | P1 | `ResidentClient`, `ResidentServer`, `ResidentFlushPolicy` | resident wire and at-most-once contract | 已固定 request/response 字段、参数读取、连接成功后服务端断开时返回 null 且同一 mutation 只发送一次；resident lifecycle/batch integration 固定 save/close、空闲可见性和 envelope；超长消息、空响应、连接失败重试次数和多 client 并发仍待补；见 `tests/OfficeCli.Tests/Unit/WordResidentWireTests.cs`、`tests/OfficeCli.Tests/Integration/WordResidentLifecycleTests.cs` | 部分自动覆盖 |
| WORD-UNIT-116 | unit | P1 | `RawXmlHelper`, `WordHandler` | raw URI and repair lifecycle | 已固定 semantic/zip URI 的 query/fragment 归一化、`.xml`/`.rels`/manifest 识别、namespace XPath 写回、typed part 读取、strict attribute repair 和失败打开后的 file handle 释放；未存在 part、manifest/非 typed part 读写、repair 后二次打开和更多 XPath action 仍待补；见 `tests/OfficeCli.Tests/Unit/WordValidationAndRawXmlTests.cs`、`WordStrictAttributeSanitizerTests.cs`、`WordRawAndFindReplaceTests.cs` | 部分自动覆盖 |
...
| WORD-CMD-042 | e2e | P1 | `src/officecli/CommandBuilder.View.cs` | view 高成本分支 | `view forms` 的 protection/fields/sdt JSON 结构和未知 mode 的 `invalid_value` envelope 已固定；`--page-count` 降级和 resident-lock 处理仍待补；screenshot 的 grid/native 基础错误边界见 WORD-CMD-047；见 `tests/OfficeCli.Tests/E2E/WordViewSmokeTests.cs` | 部分自动覆盖 |
...
| WORD-CMD-047 | e2e | P2 | `src/officecli/CommandBuilder.View.cs` | `view screenshot` 输出与降级 | screenshot 指定页/尺寸、HTML `--grid` 与当前宿主 native 不可用错误已固定；Windows + Word native、resident-lock 组合仍待补；见 `tests/OfficeCli.Tests/E2E/WordViewSmokeTests.cs` | 部分自动覆盖 |
...
| WORD-NEG-015 | e2e | P1 | `src/officecli/CommandBuilder.cs`, `ResidentClient`, `ResidentServer` | resident 锁竞争、busy、stale marker 清理 | resident 占用时 `create --force` 返回 `file_locked`；resident 异常退出后 pipe 不再可达且下一次 create 可恢复；merge/raw busy 和残留 marker 矩阵仍待补；见 `tests/OfficeCli.Tests/E2E/WordViewSmokeTests.cs` | 部分自动覆盖 |
...
| ID | 层级 | 优先级 | 依据 | 操作 / 场景 | 建议断言 | 状态 |
|---|---|---:|---|---|---|---|
| WORD-PLAT-001 | e2e | P1 | `.github/workflows/build.yml`, `src/officecli/officecli.csproj` | macOS/Windows 发布产物 Word 等价 smoke | `osx-arm64`、`win-x64` self-contained/single-file 分别执行 create/add/get/view/validate/close；同一 fixture 的 JSON 关键字段和 exit code 等价；禁止隐式 resident 影响结果 | 部分自动覆盖 |
...
| WORD-CMD-053 | e2e | P0 | `CommandBuilder.Add.cs`, `WordHandler.Mutations.cs` | remove/move/copy/swap 组合矩阵 | 已固定 body 同 parent 的 move、`add --from` copy、swap、remove 顺序，以及无效 move/swap/remove 的语义 XML 不变和最终 validate；table/header/footer、跨 parent、自移动、`sectPr` 保护和 package bytes 矩阵仍待补；见 `tests/OfficeCli.Tests/Integration/WordMutationCommandIntegrationTests.cs` | 部分自动覆盖 |
| WORD-CMD-054 | e2e | P0 | `CommandBuilder.Check.cs`, OpenXML validator | validate 诊断矩阵 | 已固定健康文档 `validate --json` 返回 0，以及 schema-invalid child 返回非零；dangling relationship、缺失 part、坏 content type、空 chart、坏 field/marker、多条错误排序和 stdout/stderr envelope 仍待补；见 `tests/OfficeCli.Tests/Integration/WordValidateCommandIntegrationTests.cs`、`WordValidationAndRawXmlTests.cs` | 部分自动覆盖 |
| WORD-CMD-055 | integration | P1 | `CommandBuilder.Dump.cs`, `WordBatchEmitter` | dump 输入路径、格式和确定性 | 已固定 `--out file` 输出 bare array、重复 dump 字节确定性和非法 subtree 不创建输出文件；`/body`/带 index 子树、`--out -`、不支持 format、warning 与 json/text envelope 的完整矩阵仍待补；见 `tests/OfficeCli.Tests/Integration/WordDumpCommandIntegrationTests.cs`、`WordDumpBatchRoundTripTests.cs` | 部分自动覆盖 |
| WORD-CMD-056 | integration | P1 | `CommandBuilder.Raw.cs`, `WordHandler.Add.Media.cs` | raw/add-part relationship 矩阵 | 已固定 CLI `add-part chart` 的 part 创建和空 chart 的 schema 诊断、`raw-set` namespaced attribute 持久化，以及非法 XPath 的非零退出与正文 XML 不变；settings/styles/header/footer/zip URI、OLE/picture、重复 relId、content type、非法 XML 和 warning 的完整矩阵仍待补；见 `tests/OfficeCli.Tests/Integration/WordRawCommandIntegrationTests.cs`、`WordRawContractTests.cs`、`WordValidationAndRawXmlTests.cs` | 部分自动覆盖 |
| WORD-CMD-057 | e2e | P1 | `CommandBuilder.View.cs` | view 全模式与参数矩阵 | 已固定 text/annotated/outline/stats/issues/forms 的 JSON envelope，以及 text 的 `--max-lines`、annotated 的 start/end、issues 的 limit、HTML 的 page/out 文件和非法 render；screenshot/grid/page-count/native renderer 与完整裸输出矩阵仍待补；见 `tests/OfficeCli.Tests/Integration/WordViewCommandIntegrationTests.cs` | 部分自动覆盖 |
| WORD-CMD-058 | e2e | P1 | `CommandBuilder.Watch.cs`, `WatchServer.cs` | watch HTTP/SSE 生命周期 | 已固定真实 WatchServer 的 marker/loopback HTML、SSE 初始 selection 事件、selection POST、错误 API/Host 拒绝，以及 StopAsync 后 marker 清理；health/static resource、mutation 后 SSE 更新、断开重连和 CLI watch/unwatch 进程生命周期仍待补；见 `tests/OfficeCli.Tests/Integration/WordWatchServerIntegrationTests.cs` | 部分自动覆盖 |
...
| WORD-CMD-061 | integration | P0 | `ResidentClient.cs`, `ResidentServer.cs` | resident 并发与 at-most-once | 已固定 8 个并发 client 对非幂等 add 的串行处理、每个结果只落一个顶层 paragraph、save 后重开仍恰好 8 个；set/remove/batch、busy/retry、空响应和崩溃恢复仍待补；见 `tests/OfficeCli.Tests/Integration/WordResidentConcurrencyTests.cs` | 部分自动覆盖 |
...
| WORD-CMD-063 | e2e | P1 | `CommandBuilder.Import.cs` | merge 模板和多 part token 矩阵 | 已固定 Word body 的 inline JSON、嵌套对象/数组 placeholder、未解析 placeholder 保留、`.json` 数据文件、已有输出拒绝和 `--force` 覆盖、非法 JSON 不创建输出；多 part、跨 run、重复 placeholder、转义花括号、完整错误 envelope 仍待补；见 `tests/OfficeCli.Tests/Integration/WordMergeCommandIntegrationTests.cs` | 部分自动覆盖 |
...
| WORD-DOC-028 | contract | P1 | `paragraph.json`, `run.json` | paragraph/run 高级属性矩阵 | 已固定段落 keepNext/keepLines/pageBreakBefore/widowControl、RTL、pattern shading，以及 run 的脚本字体、语言、toggle、效果和文本边界；tabs、theme/effect 全量、mark-run language 和更多 alias 仍待补；见 `tests/OfficeCli.Tests/Unit/WordParagraphTests.cs`、`WordRunTests.cs`、`WordTabContractTests.cs` | 部分自动覆盖 |
| WORD-DOC-029 | contract | P1 | table/row/cell schemas | table complete contract | 已固定 table/row/cell 的 layout、merge、height/header/cantSplit、width、padding、fitText、nowrap、textDirection、shading 等主要属性，以及 row/cell 删除和 readback；border/spacing/indent、默认值、alias 和非法组合全量仍待补；见 `tests/OfficeCli.Tests/Unit/WordTableTests.cs`、相关 table contract | 部分自动覆盖 |
| WORD-DOC-030 | contract | P1 | picture/ole/chart/diagram/equation/shape/textbox/watermark schemas | media/visual complete contract | 已覆盖 picture/OLE/chart/diagram/equation/shape/textbox/watermark 的主要必填、尺寸、位置、关系、query/get/remove、validate 和部分 dump replay；HTML parity、复杂 carrier、每类 add-only/settable 与错误矩阵仍待补；见 `tests/OfficeCli.Tests/Unit/WordChartDiagramPreviewTests.cs`、`WordComplexObjectTests.cs`、`WordMediaQueryTests.cs`、相关 functional/integration tests | 部分自动覆盖 |
| WORD-DOC-031 | contract | P1 | field/toc/formfield/sdt schemas | field/control complete contract | 已覆盖 PAGE/NUMPAGES/SEQ/REF/TOC 的部分 parser/cache、formfield 基础行为和 SDT 的主要 type/lock/checked/value/readback；所有 control type、placeholder/list/date、TOC refresh 和只读边界全量仍待补；见 `tests/OfficeCli.Tests/Unit/WordFieldParserTests.cs`、`WordFieldAndFormTests.cs`、`WordSdtTests.cs`、相关 contract | 部分自动覆盖 |
| WORD-DOC-032 | integration | P1 | package parts and relationship graph | unknown part preservation | 已固定 customXml/customXmlProperties/fontTable/webSettings 与 embedded OLE 在局部 mutation、dump/replay 后保留，且 paragraph/readback/validate 通过；theme、core/app/custom docProps、comments/notes/header/footer、content type 和 binary bytes 全量仍待补；见 `tests/OfficeCli.Tests/Integration/WordDocumentFidelityTests.cs`、`WordDumpBatchRoundTripTests.cs` | 部分自动覆盖 |
| WORD-DOC-033 | integration | P1 | `WordBatchEmitter`, `BatchExecutor` | full dump replay idempotency | 已固定 full dump 到两个 fresh docx 的 replay 结果产生相同第二次 dump、资源顺序和 validate 结果；stable IDs、复杂关系、重复 section/part/relationship 和同一目标二次 replay 的全量矩阵仍待补；见 `tests/OfficeCli.Tests/Integration/WordDumpBatchRoundTripTests.cs`、`WordComplexDumpCarrierRoundTripTests.cs` | 部分自动覆盖 |
| WORD-DOC-034 | integration | P1 | `examples/word/*` | 示例全量 smoke 分层 | 已自动执行 `content-controls.sh`、`pictures.sh`、`revisions.sh`、`tables.sh` 四个代表性 shell 示例，并断言 docx 可生成、非空、可重新打开且包含 paragraph；其余 shell 示例、全部 Python SDK 示例、每类示例的专属结果断言仍待补；见 `tests/OfficeCli.Tests/Integration/WordExampleScriptSmokeTests.cs` | 部分自动覆盖 |
| WORD-DOC-035 | contract | P2 | `WordHandler.HtmlPreview.*`, rendering registry | HTML/render contract | 已固定文本/attribute 安全转义、图片 data URI 降级、table/paragraph/picture 基础 HTML、renderer capability 和部分 view range/grid；path/anchor/class、field/revision/comment/note marker、page 输出和 capability 缺失的完整降级矩阵仍待补；见 `tests/OfficeCli.Tests/Unit/WordHtmlSafetyTests.cs`、`WordHtmlRefreshTests.cs`、`WordRendererRegistryTests.cs`、`WordHtmlPreviewContractTests.cs` | 部分自动覆盖 |
| WORD-DOC-036 | contract | P1 | revision/permission/protection handlers | revision、permission、protection 交叉行为 | 已固定 run/paragraph/table/cell revision marker、accept/reject、permission range 和非法输入的主要行为；tracked move/set/remove 与 protection/force/allowed-range、batch/resident 交叉矩阵仍待补；见 `tests/OfficeCli.Tests/Functional/WordRevisionContractTests.cs`、`WordPermissionContractTests.cs`、`tests/OfficeCli.Tests/Unit/WordRevisionTests.cs`、`WordBreakTabPermissionTests.cs` | 部分自动覆盖 |

### 负向、资源和平台补充矩阵

| ID | 层级 | 优先级 | 依据 | 操作 / 场景 | 建议断言 | 状态 |
|---|---|---:|---|---|---|---|
| WORD-NEG-018 | contract | P0 | `CommandBuilder.*`, `MutationSelectorGuard` | CLI 参数和 selector 解析失败 | 已固定 add 缺 type、`--index/--before` 互斥、空 prop key、`--from`/`--prop` 冲突，以及 set/remove bare selector 的非零退出和语义 XML 不变；坏 bool/number/length/color、未闭合 selector、shell quote/path bracket、完整 code/stderr 矩阵仍待补；见 `tests/OfficeCli.Tests/Integration/WordMutationCommandIntegrationTests.cs` | 部分自动覆盖 |
...
| WORD-NEG-026 | integration | P1 | retry/close/save/batch semantics | 重复执行与幂等边界 | 已固定无 resident 时重复 save、close、validate 均成功且 package bytes 不变；同一 batch 重放、响应丢失后的重试、已有 resident 的 open/create force 占用和非幂等 mutation 仍待补；见 `tests/OfficeCli.Tests/Integration/WordMutationCommandIntegrationTests.cs` | 部分自动覆盖 |
| WORD-MEDIA-028 | regression | P1 | `OleHelper`, `WordHandler.Add.Media` | OLE/embedded package full round-trip | 已固定 package/legacy object 的 content type、progId、binary payload、display/size、dump/batch 基础 round-trip 和删除后的 validate；copy/remove、复杂 embedded package 与完整 relationship graph 仍待补；见 `tests/OfficeCli.Tests/Unit/WordMediaQueryTests.cs`、`WordOleContractTests.cs`、`WordDumpBatchRoundTripTests.cs` | 部分自动覆盖 |
| WORD-MEDIA-029 | regression | P1 | chart builders and resources | chart externalData/workbook round-trip | 已固定 native chart 的 cache/series/category refs、range-backed data、style/external relationship carrier 和 host relId 基础回放；embedded workbook、externalData 变更、复制/删除和全 content type 关系矩阵仍待补；见 `tests/OfficeCli.Tests/Functional/WordChartContractTests.cs`、`tests/OfficeCli.Tests/Unit/WordChartDiagramPreviewTests.cs`、`WordComplexDumpCarrierRoundTripTests.cs` | 部分自动覆盖 |
| WORD-MEDIA-030 | regression | P1 | complex carrier handlers | ActiveX/VML/diagram/equation/rich SDT carrier | 已固定最小 ActiveX `<w:control>` + activeX binary 的 dump→batch replay：关系 ID 重写、控制 XML/嵌套 part 保留并可重新打开、validate 通过；VML 外部关系已有局部 fixture；VML preview、diagram/equation OLE/rich SDT/embedded font/numPicBullet 和复杂 carrier 矩阵仍待补；见 `tests/OfficeCli.Tests/Integration/WordActiveXCarrierRoundTripTests.cs`、`tests/OfficeCli.Tests/Unit/WordMediaQueryTests.cs` | 部分自动覆盖 |
| WORD-MEDIA-031 | regression | P1 | resource emitter and package graph | resource ordering and content types | 已固定 full dump 中 styles、theme、settings、core/app/custom docProps 在 body add 之前输出，且 docProps 顺序稳定；customXml/fontTable/webSettings/embedded parts、warning、content type 和完整 replay 顺序仍待补；见 `tests/OfficeCli.Tests/Integration/WordDumpResourceOrderTests.cs`、`WordDocumentFidelityTests.cs` | 部分自动覆盖 |
| WORD-REF-025 | e2e | P1 | `CommandBuilder.Refresh.cs`, `WordHtmlRefresh` | refresh field result matrix | 已固定 HTML refresh 的 PAGEREF anchor 提取、已知 anchor cache 更新、未知 PAGEREF 和 PAGE cache 不被误改；TOC/PAGE/NUMPAGES/SEQ/REF 完整构造、dirty/updateFields、Pages 和 native/fallback envelope 仍待补；见 `tests/OfficeCli.Tests/Unit/WordHtmlRefreshTests.cs` | 部分自动覆盖 |
| WORD-REF-026 | contract | P1 | `sdt.json`, `formfield.json` | controls complete matrix | 已覆盖 text/checkbox/dropdown/formfield 的主要 value/result/checked/lock/placeholder/readback、set/remove 和部分 dump replay；combobox/date/picture/group/richtext、list/default state 和全量只读边界仍待补；见 `tests/OfficeCli.Tests/Unit/WordSdtTests.cs`、`WordFieldAndFormTests.cs`、相关 SDT/formfield contract | 部分自动覆盖 |
| WORD-REF-027 | contract | P1 | `hyperlink.json`, `WordHandler` | hyperlink target matrix | 已覆盖 external URL、fragment anchor、bookmark、run formatting、set/remove 和基础关系清理；theme/inherit color、invalid target、跨 host relationship 与完整 replay 矩阵仍待补；见 `tests/OfficeCli.Tests/Functional/WordHyperlinkContractTests.cs`、`tests/OfficeCli.Tests/Unit/WordRawAndFindReplaceTests.cs` | 部分自动覆盖 |
| WORD-REF-028 | contract | P1 | comment/note handlers | comment/note range and format matrix | 已覆盖 comment/footnote/endnote 的主要 range、文本、格式、删除和 validate 行为，部分 commentsExtended metadata 已由 functional 固定；thread/reply/done/parentId 全量、跨 part dump replay、marker/part 清理矩阵仍待补；见 `tests/OfficeCli.Tests/Unit/WordNotesAndCommentsTests.cs`、`tests/OfficeCli.Tests/Functional/WordNotesAndCommentsContractTests.cs` | 部分自动覆盖 |
| WORD-PLAT-003 | e2e | P0 | publish workflow and `WordPublishedBinarySmokeTests` | publish artifact execution matrix | 已在当前宿主机发布并验证 `osx-arm64`、`win-x64` self-contained 产物存在且体积合理；当前 OS 可执行产物已覆盖 create/add/view/validate、exit code 和 JSON 字段，另一目标 OS 的真实启动、权限和依赖仍待在对应 CI runner 固定；见 `tests/OfficeCli.Tests/E2E/WordPublishedBinarySmokeTests.cs` | 部分自动覆盖 |
| WORD-PLAT-005 | e2e | P1 | refresh/render backends | native Word 与 fallback | 已固定当前 macOS 上 HTML/text view、refresh 的 fallback 行为和 native renderer 不可用时的稳定失败边界，并覆盖 screenshot 的部分参数路径；Windows + Word native、Windows 无 Word、Linux fallback、页面数和全部输出文件规则仍待在对应环境固定；见 `tests/OfficeCli.Tests/E2E/WordViewSmokeTests.cs`、`tests/OfficeCli.Tests/Integration/WordViewCommandIntegrationTests.cs` | 部分自动覆盖 |
| WORD-PLAT-006 | e2e | P1 | locale/path runtime | locale/path matrix | 已固定 `zh-CN`、`ar-SA`、CJK/RTL/emoji 文本、含空格和非 ASCII 路径的 create/readback/validate；`ja-JP`/`en-US`、长路径、Windows separator 及发布包跨 OS 等价性仍待补；见 `tests/OfficeCli.Tests/E2E/WordViewSmokeTests.cs`、`tests/OfficeCli.Tests/Integration/WordFilesystemBoundaryIntegrationTests.cs` | 部分自动覆盖 |
...
| WORD-UNIT-120 | unit | P0 | `src/officecli/Core/DocumentLimits.cs`, `DocumentHandlerFactory.cs` | OOXML zip 资源保护 | 已固定 `DocumentLimits` 的 entry/size/ratio/depth/regex 常量、`EnsureDepth` 边界、`max_depth_exceeded`、0-byte 文件拒绝和超额 zip entry 的 `decompression_bomb`；压缩比/总解压大小、repair 后文件不变性仍待补；见 `tests/OfficeCli.Tests/Unit/WordResourceLimitTests.cs` | 部分自动覆盖 |
...
| WORD-UNIT-124 | unit | P1 | `src/officecli/Core/WordHtmlRefresh.cs` | refresh backend 生命周期 | 已固定 PAGEREF anchor 提取、已知 anchor 的 cached page result 写回、缺失/损坏文档返回 false 且不改源文件；浏览器 pagination 成功/失败、临时 HTML 清理、field dirty/updateFields、native/fallback backend 选择和 timeout 仍待补；见 `tests/OfficeCli.Tests/Unit/WordHtmlRefreshTests.cs` | 部分自动覆盖 |
...
| WORD-CMD-070 | e2e | P1 | `CommandBuilder.View.cs`, rendering registry | view renderer 选择与输出冲突 | html/svg/screenshot 的 backend capability、`--render`、`--out`/stdout、已有输出、page filter/grid 和未知 renderer 的 JSON/裸文本错误边界固定 | 部分自动覆盖 |
