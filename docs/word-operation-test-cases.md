...
| WORD-CMD-057 | e2e | P1 | `CommandBuilder.View.cs` | view 全模式与参数矩阵 | 已固定 text/annotated/outline/stats/issues/forms 的 JSON envelope，以及 text 的 `--max-lines`、annotated 的 start/end、issues 的 limit、HTML 的 page/out 文件和非法 render；screenshot/pdf/grid/page-count/native renderer 与完整裸输出矩阵仍待补；见 `tests/OfficeCli.Tests/Integration/WordViewCommandIntegrationTests.cs` | 部分自动覆盖 |
...
| WORD-CMD-064 | integration | P1 | `CommandBuilder.Plugins.cs`, MCP server | plugin/MCP 错误协议矩阵 | 已固定 dump-reader JSONL replay、顶层 array 的 `corrupt_batch`、plugins lint 成功/失败，以及 manifest unknown kind、非法 target、非法 idle timeout 的 warning 和 60 秒安全 fallback；非 dump-reader、超时/非零退出/坏 JSON/heartbeat、MCP JSON-RPC 与 stderr 纯度仍待补；见 `tests/OfficeCli.Tests/Integration/WordPluginContractTests.cs` | 部分自动覆盖 |
| WORD-CMD-065 | e2e | P1 | `Program.cs`, `McpInstaller`, update/install guards | 自动安装、更新和环境隔离 | 已固定子进程在 `OFFICECLI_NO_AUTO_INSTALL=1`、`OFFICECLI_SKIP_UPDATE=1` 下执行 Word `validate --json` 成功，stdout 可解析且 stdout/stderr 不含安装或更新日志；无配置/只读目录、无网络、插件目录隔离和 MCP 注册副作用仍待补；见 `tests/OfficeCli.Tests/Integration/WordPluginContractTests.cs` | 部分自动覆盖 |
...
| WORD-PLAT-004 | e2e | P1 | installer/update/runtime guards | bundled-agent isolation | 已固定 Word 子进程和发布 smoke 使用 `OFFICECLI_NO_AUTO_INSTALL`、`OFFICECLI_SKIP_UPDATE`、`OFFICECLI_NO_AUTO_RESIDENT` 时 stdout/stderr 不混入安装或更新日志，且 JSON 协议可解析；宿主 agent 工作目录、stdin、resident pipe、临时文件和插件注册隔离仍待补；见 `tests/OfficeCli.Tests/Integration/WordPluginContractTests.cs`、`tests/OfficeCli.Tests/E2E/WordPublishedBinarySmokeTests.cs` | 部分自动覆盖 |
| WORD-PLAT-005 | e2e | P1 | refresh/render backends | native Word 与 fallback | 已固定当前 macOS 上 HTML/text view、refresh 的 fallback 行为和 native renderer 不可用时的稳定失败边界，并覆盖 screenshot/pdf 的部分参数路径；Windows + Word native、Windows 无 Word、Linux fallback、页面数和全部输出文件规则仍待在对应环境固定；见 `tests/OfficeCli.Tests/E2E/WordViewSmokeTests.cs`、`tests/OfficeCli.Tests/Integration/WordViewCommandIntegrationTests.cs` | 部分自动覆盖 |
