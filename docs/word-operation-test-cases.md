...
本轮整理后的规模统计：操作/场景条目共 364 条，其中已有单元覆盖 102 条、已有自动覆盖 97 条、部分自动覆盖 123 条、示例待改造 14 条、建议人工核验 2 条、CI smoke 1 条，新增待补 25 条；另有 154 条 `WORD-FREEZE-*` 冻结基线映射。这里的条目数不是 xUnit 方法数，也不是展开后的 test case 数，后续实现时一个条目可以对应一个或多个测试方法/数据行。
